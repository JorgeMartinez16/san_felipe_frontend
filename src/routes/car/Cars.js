...
const Cars = () => {
  ...
  const handleSearchCar = async (e) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:8080/cars/${searchPlate}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const handleDeleteCar = async (licencePlate) => {
    try {
      const response = await fetch(
        `http://localhost:8080/cars/${licencePlate}`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${token}`,
            ...
            {cars.length > 0 ? (
              cars.map((car) => (
                <tr key={car.licencePlate}>
                  ...
                  <td>
                    <button
                      onClick={() => handleDeleteCar(car.licencePlate)}
              ...
              <tr>
                <td colSpan="4" style={{ textAlign: "center" }}>
                  {isSearching
                    ? "No se encontró ningún auto con esa placa"
                    : "AUTOS LISTADOS"}
                </td>
              </tr>
            )}
